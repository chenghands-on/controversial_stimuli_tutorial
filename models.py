...
import timm  # 添加timm库用于ResNeXt101_32x32d模型
...
class TVPretrainedModel(torch.nn.Module):
  ...
  def load(self,device=None):
    ...
    """
    if "resnext101_32x32d" in self.model_name:
      # 使用timm加载ResNeXt模型
      self.add_module("net", timm.create_model(self.model_name, pretrained=True))
    else:
      self.add_module("net", getattr(tv.models, self.model_name)(pretrained=True))
...
class Resnet50(TVPretrainedModel):
    def __init__(self):
      super().__init__('resnet50',224)

class InceptionV3(TVPretrainedModel):
    def __init__(self):
      super().__init__('inception_v3',299) # note the different input-image size

# 新增：ViT模型类
class ViTB16(TVPretrainedModel):
    def __init__(self):
      super().__init__('vit_b_16',224)

class ViTL16(TVPretrainedModel):
    def __init__(self):
      super().__init__('vit_l_16',224)

# 新增：ResNeXt模型类
class ResNeXt101_32x32d(TVPretrainedModel):
    def __init__(self):
      super().__init__('resnext101_32x8d',224)  # 修正为torchvision中实际可用的模型名称

    def load(self, device=None):
      """Override the load method to use torchvision instead of timm for this model"""
      self.add_module("net", getattr(tv.models, self.model_name)(pretrained=True))
      if device is None:
        if torch.cuda.is_available():
          self.device=torch.device('cuda',0)
